  ...
  const handleFiles = useCallback((files) => {
    ...
    const supportedFiles = files.filter(file => {
      ...
      const imageExtensions = [
        ...
        // '.tiff', '.tif', '.avif', '.heic', '.heif', '.svg', '.hdr', '.exr'
      ];
      ...
      const meshExtensions = [
        '.obj', '.ply', '.gltf', '.glb'
        // Advanced formats commented out until Phase 4:
        // '.stl', '.fbx', '.dae', '.3ds', '.x3d', '.usdz'
      ];
    ...
    if (supportedFiles.length === 0) {
      alert('Please select valid files:\n• Images: PNG, JPEG, WebP, GIF, BMP, ICO\n• 3D Models: OBJ, PLY, GLTF, GLB\n\n(Advanced formats like AVIF, HEIC, SVG, STL, FBX will be supported in future phases)');
      return;
    }
    ...
    <div className="rounded-sm overflow-hidden">
      ...
      >
        ...
        <input
          ...
          type="file"
          multiple
          accept=".png,.jpg,.jpeg,.gif,.webp,.bmp,.ico,.obj,.ply,.gltf,.glb"
