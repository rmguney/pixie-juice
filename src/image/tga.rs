...
pub fn optimize_tga_entry(data: &[u8], quality: u8) -> Result<Vec<u8>, wasm_bindgen::JsValue> {
    ...
    match optimize_tga(data, quality) {
        Ok(optimized_data) => {
...
pub fn optimize_tga(data: &[u8], quality: u8) -> PixieResult<Vec<u8>> {
    ...
    #[cfg(feature = "image")]
    {
        if let Ok(img) = image::load_from_memory_with_format(data, image::ImageFormat::Tga) {
...
fn optimize_tga_with_basic_processing(data: &[u8], config: &ImageOptConfig) -> PixieResult<Vec<u8>> {
    ...
    match bit_depth {
        32 => convert_tga_to_png_simple(data, config),
        _ => optimize_16bit_tga_in_place(data, config),
    }
}
