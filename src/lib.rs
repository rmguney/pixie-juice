...
pub fn optimize_avif(data: &[u8], quality: u8) -> Result<Vec<u8>, JsValue> {
    crate::image::avif::optimize_avif_entry(data, quality)
}
...
#[wasm_bindgen]
pub fn convert_to_avif(data: &[u8], quality: u8) -> Result<Vec<u8>, JsValue> {
    // AVIF encoding requires C libraries not compatible with WASM
    let _ = (data, quality);
    Err(JsValue::from_str("AVIF encoding not supported in WASM environment. Use convert_to_webp for similar compression."))
}
