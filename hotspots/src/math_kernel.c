...
// Fast WASM math implementations using optimized algorithms

// Fast reciprocal square root (Quake algorithm adapted for WASM)
...
// Fast square root using reciprocal square root
static inline float fast_sqrt(float x) {
    if (x <= 0.0f) return 0.0f;
    return x * fast_inv_sqrt(x);
}

// Fast sine using Bhaskara I approximation with range reduction
...
// Fast cosine using sin(x + π/2) = cos(x)
static inline float fast_cos(float x) {
    return fast_sin(x + M_PI_2_F);
}

// Fast tangent using sin/cos with special case handling
static inline float fast_tan(float x) {
    float s = fast_sin(x);
    float c = fast_cos(x);
    return (c == 0.0f) ? 1e30f : s / c; // Avoid division by zero
}

// Fast exponential using polynomial approximation
static inline float fast_exp(float x) {
    if (x > 88.0f) return 1e30f; // Prevent overflow
    if (x < -88.0f) return 0.0f;
    
    // Use exp(x) = exp(i + f) = exp(i) * exp(f) where i is integer part
    int i = (int)x;
    float f = x - (float)i;
    
    // Calculate exp(f) using Taylor series (f is small)
    float exp_f = 1.0f + f * (1.0f + f * (0.5f + f * (0.16666667f + f * 0.04166667f)));
    
    // Calculate exp(i) using repeated squaring for powers of e
    float exp_i = 1.0f;
    float base = M_E_F;
    int exp = i < 0 ? -i : i;
...
// Fast natural logarithm using bit manipulation and polynomial
static inline float fast_log(float x) {
    ...
    // Polynomial approximation for ln(1+y)
    float y2 = y * y;
    float y3 = y2 * y;
...
// Fast power function using exp(y * ln(x))
...
// Color space conversions with optimized formulas
...
void hsv_to_rgb(float h, float s, float v, float* r, float* g, float* b) {
    ...
    h = h - ((int)(h / 360.0f)) * 360.0f; // Normalize to [0, 360)
    if (h < 0.0f) h += 360.0f;
...
void simd_vec4_add(const float* a, const float* b, float* result) {
    if (!a || !b || !result) return;
...
void simd_vec4_multiply(const float* a, const float* b, float* result) {
    if (!a || !b || !result) return;
...
void simd_matrix4_multiply(const float* a, const float* b, float* result) {
    if (!a || !b || !result) return;
...
void simd_color_convert_batch(const float* rgb_array, float* hsv_array, size_t count) {
    if (!rgb_array || !hsv_array) return;
    
    // Process 4 RGB triplets at once using SIMD
    size_t simd_count = (count / 4) * 4;
    
    for (size_t i = 0; i < simd_count; i += 4) {
        // Load 4 RGB values (12 floats)
        ...
        // Hue calculation is more complex, fall back to scalar for now
