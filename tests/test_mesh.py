...
async def setup_page_logging(page):
    ...
    def handle_response(response):
        if any(keyword in response.url.lower() for keyword in ['wasm', 'pixie', 'optimize']):
            ...
            print(f"   🌐 Response: {status_icon} {response.status} {response.url}")
...
async def run_comprehensive_mesh_test():
    ...
    print("🧪 Pixie Juice Comprehensive Mesh Testing Suite")
    print("=" * 60)
    ...
    async with async_playwright() as p:
        ...
        print("🔧 Setting up browser console logging...")
        ...
        try:
            ...
            test_cases = [
                ...
                ("OBJ_Cube", "fixtures/meshes/obj/simple_cube.obj", 15),
                ("OBJ_Sphere_Medium", "fixtures/meshes/obj/medium_sphere.obj", 20),
                ("OBJ_Sphere_Complex", "fixtures/meshes/obj/complex_sphere.obj", 30),
                ("OBJ_Grid_Large", "fixtures/meshes/obj/large_grid.obj", 40),
                
                # STL formats - using generated fixtures
                ("STL_ASCII_Simple", "fixtures/meshes/stl/simple_ascii.stl", 10),
                ("STL_Binary_Simple", "fixtures/meshes/stl/simple_binary.stl", 12),
                ("STL_Binary_Medium", "fixtures/meshes/stl/medium_binary.stl", 18),
                ("STL_Binary_Large", "fixtures/meshes/stl/large_binary.stl", 25),
                
                # PLY formats - using generated fixtures
                ("PLY_ASCII_Simple", "fixtures/meshes/ply/simple_quad.ply", 15),
                ("PLY_Binary_Medium", "fixtures/meshes/ply/medium_binary.ply", 20),
                ("PLY_Binary_Large", "fixtures/meshes/ply/large_binary.ply", 30),
                
                # glTF formats - using generated fixtures
                ("GLTF_Simple", "fixtures/meshes/gltf/simple_triangle.gltf", 20),
                
                # GLB formats - using generated fixtures
                ("GLB_Simple", "fixtures/meshes/glb/simple_triangle.glb", 22),
                
                # FBX formats - using generated fixtures
                ("FBX_Simple", "fixtures/meshes/fbx/simple_mesh.fbx", 25),
                ("FBX_Medium", "fixtures/meshes/fbx/medium_mesh.fbx", 30),
                ("FBX_Complex", "fixtures/meshes/fbx/complex_mesh.fbx", 40),
                
                # Legacy fixtures (fallback)
                ("Legacy_OBJ_Simple", "fixtures/meshes/simple.obj", 18),
                ("Legacy_STL_Sample", "fixtures/meshes/sample.stl", 15),
            ]
            ...
            if not available_test_cases:
                print("❌ No mesh test files available!")
                return test_results
            
            print(f"\n🎯 Running mesh tests on {len(available_test_cases)} available files...")
            ...
            for format_name, fixture_path, expected_optimization in available_test_cases:
                ...
                try:
                    ...
                    if console_logger.messages:
                        print(f"   📝 Console activity after upload: {len(console_logger.messages)} messages")
                    ...
                    if await quality_slider.count() > 0:
                        print(f"   🎚️ Setting quality to 75%...")
                    ...
                    if await optimize_btn.count() > 0:
                        ...
                        print(f"   📝 Console activity during optimization: {optimization_messages} new messages")
                        
                        if not result_panel_visible:
                            # Debug UI content when result panel is not found
                            print("   🔍 Result panel not found, debugging UI content...")
                        ...
                        if optimization_ratio:
                            print(f"   🔧 Optimization: {optimization_ratio:.1f}% (expected: ≥{expected_optimization}%) {'✅' if optimization_ratio >= expected_optimization else '❌'}")
                        else:
                            print(f"   ⚠️  Could not extract optimization ratio from UI")
            ...
            if cube_obj.exists():
                ...
                for quality in [25, 50, 75, 90]:
                    try:
                        print(f"\n   📊 Testing Quality {quality}% (mesh reduction ratio)...")
                        ...
                        quality_slider = page.locator('input[type="range"]')
                        if await quality_slider.count() > 0:
                            print(f"      🎚️ Setting quality to {quality}%...")
                        ...
                        optimize_btn = page.locator('button:has-text("Optimize")')
                        if await optimize_btn.count() > 0:
                            print(f"      🚀 Starting mesh optimization at quality {quality}%...")
                            ...
                            # Log optimization console activity
                            optimization_messages = len(console_logger.messages) - optimization_start_messages
                            print(f"      📝 Console activity: {optimization_messages} messages")
                            ...
                            if console_logger.errors or console_logger.warnings:
                                print(f"      📝 Console: {len(console_logger.errors)} errors, {len(console_logger.warnings)} warnings")
    ...
    # Save results
    results_file = "test_mesh_results.json"
    ...
    if console_logger:
        ...
        print(f"\n📝 Complete Browser Console Summary:")
        print(f"   Total messages: {console_summary['total_messages']}")
        print(f"   Errors: {console_summary['error_count']} ❌")
        print(f"   Warnings: {console_summary['warning_count']} ⚠️")
        print(f"   Info/Debug: {console_summary['info_count']} ℹ️")
...
# Pytest-compatible version of the test
import pytest
