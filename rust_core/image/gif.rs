...
pub fn optimize_gif(data: &[u8], config: &OptConfig) -> OptResult<Vec<u8>> {
    ...
    // Re-encode the GIF with advanced color reduction
    if config.reduce_colors.unwrap_or(false) {
        #[cfg(feature = "c_hotspots")]
        {
            ...
            // Fallback to simple re-encoding for now
        ...
        {
            // Fallback quantization using image crate conversion
            let mut png_data = Vec::new();
            let mut png_cursor = std::io::Cursor::new(&mut png_data);
            img.write_to(&mut png_cursor, image::ImageFormat::Png)
                .map_err(|e| OptError::ProcessingError(format!("Failed to encode PNG intermediate: {}", e)))?;
            
            // Re-decode and encode as GIF
            let quantized_img = image::load_from_memory(&png_data)
                .map_err(|e| OptError::ProcessingError(format!("Failed to decode PNG intermediate: {}", e)))?;
            
            quantized_img.write_to(&mut cursor, image::ImageFormat::Gif)
                .map_err(|e| OptError::ProcessingError(format!("Failed to encode optimized GIF: {}", e)))?;
        }
    } else {
        // Simple re-encoding without color reduction
        img.write_to(&mut cursor, image::ImageFormat::Gif)
            .map_err(|e| OptError::ProcessingError(format!("Failed to encode GIF: {}", e)))?;
    }
    
    log::info!("GIF optimized: {} bytes -> {} bytes ({:.1}% change)", 
              data.len(), 
              output.len(),
